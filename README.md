...
- **橙＝帯（自社帯に差し替え）/ 赤＝ロゴ / 青＝案内図 / 灰＝その他**（帯以外は白塗りのみ）
- 枠を **ドラッグで移動**、選択中に出る青ハンドルで **サイズ変更**、**×で削除**
- 「＋帯」「＋ロゴ」「＋案内図」「＋その他」で枠を **追加**（帯は全幅・縦のみ調整、それ以外は2D自由）
- 選択中の枠は **種類を後から変更** できる（誤ってロゴと判定された案内図を直す等）
- 「適用して貼り直す」で元画像から再構成（誤検出の削除・ズレ修正・取りこぼし追加が1画面で完結）

**種類を正しく選ぶことが再学習の質に直結する。** ロゴ・案内図は学習ラベルになるが、
「その他」（QRコードなど帯でもロゴでも案内図でもないもの）は白塗りするだけで学習には使わない。
